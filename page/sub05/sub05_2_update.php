<?php
	...
	$mb_dir = G5_DATA_PATH.'/siber/'.$file_name;
	
	
	if (isset($_FILES['file'])) {
		//  파일 용량이 설정값보다 이하만 업로드 가능
		if ($_FILES['file']['size'] <= "2097152") {
			@mkdir(G5_DATA_PATH.'/siber/', G5_DIR_PERMISSION);
			@chmod(G5_DATA_PATH.'/siber/', G5_DIR_PERMISSION);
			$dest_path = $mb_dir;
	...
	$mail_con.="<h2 style='padding-left:40px;background:url(".G5_IMG_URL."/inquiry_icon2.png) left 20px no-repeat; color:#b20000; font-size:40px;margin-bottom:25px; font-weight:bold;'>제보하기</h2>";
		...
		$mail_con.="</table>";
	$mail_con.="</div>";
	mailer($name, "0330_aa@naver.com", "0330_aa@naver.com", "사이버감사팀 제보 입니다.", $mail_con);
	mailer($name, $config['cf_admin_email'], $config['cf_admin_email'], "사이버감사팀 제보 입니다.", $mail_con);
	alert("등록 되었습니다.");
?>
